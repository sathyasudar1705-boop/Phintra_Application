...
@router.post("/auth/microsoft/verify")
def microsoft_verify(payload: MicrosoftVerifyRequest, db: Session = Depends(get_db)):
    """Verify Microsoft token or exchange auth code, locate user/employee, and issue Phintra JWT."""
    import requests
    ...
    # If code is provided instead of token, exchange it
    if payload.code:
        ...
        if res.status_code != 200:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=f"Microsoft token exchange failed: {res.text}"
            )
    ...
    if not access_token:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Either code or token must be provided"
        )
    ...
    if profile_res.status_code != 200:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Failed to retrieve Microsoft user profile"
        )
    ...
    # Check User table (Admin/Manager)
    ...
    if user:
        ...
        audit = AuditLog(user_id=user.id, action="Microsoft Login", details=f"User {user.email} logged in successfully via Microsoft Entra ID.")
        db.add(audit)
        db.commit()
        ...
        return {
            ...
            "user": {
                "id": str(user.id),
                "name": user.name or profile.get("displayName", "User"),
                "email": user.email,
                "role": user.role
            }
        }
    ...
    if emp:
        ...
        audit = AuditLog(action="Employee Microsoft Login", details=f"Employee {emp.email} logged in successfully via Microsoft Entra ID.")
        db.add(audit)
        db.commit()
        ...
        return {
            ...
            "employee": {
                "name": emp.name or profile.get("displayName", f"{emp.first_name} {emp.last_name}".strip() or "Employee"),
                "email": emp.email,
                "department": dept_name,
                "role": "Employee",
                "employee_id": str(emp.id)
